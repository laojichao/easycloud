	...
	$.get(url, function(d) {
		if(d.saveOK ==0){
			...
			$.get("login.php?act=qrlogin&r="+Math.random(1), function(arr) {
				if(arr.code==0) {
					layer.msg('<center><img src="https://q4.qlogo.cn/g?b=qq&nk='+arr.qq+'&s=100" height="60"></center><hr>你好'+arr.name+',欢迎回来！', {
...
function mloginurl(){
	...
	$('#mlogin').html("正在跳转...");
	$.post("login.php?act=qrcode&r="+Math.random(1),"image="+encodeURIComponent(imagew), function(arr) {
		if(arr.code==0) {

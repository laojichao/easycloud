	...
	<div class="layui-row layui-col-space8">
		<div class="layui-col-xs12 layui-col-sm8 layui-col-sm-offset2 layui-col-md6 layui-col-md-offset3 misett">
			<div class="layui-card">
				<div class="layui-card-header" style="font-size: 1.5rem;text-align: center;height: 3rem;line-height: 3rem;">在线更换</div>
				<div class="layui-card-body">
					<?php if ($conf['change_open'] == 1) {?>
					<div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
						<ul class="layui-tab-title" style="text-align: center;margin-bottom: 1rem;">
							<li class="layui-this">更换授权</li>
							<li>更换认证</li>
						</ul>
							...
							<div class="layui-tab-item layui-show">
									<form class="layui-form">
										<div class="layui-form-item">
											...
											<option value="0">请选择授权的程序</option>
											<?php
										...
										<div class="layui-form-item">
											<input type="text" name="authqq" placeholder="授权绑定的QQ" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<input type="text" name="authurl" placeholder="旧域名" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<input type="text" name="authurls" placeholder="新域名" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<input type="text" name="authcode" placeholder="验证码" class="layui-input" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<div class="layui-row layui-col-space12">
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color" lay-submit lay-filter="submit_authgetcode">获取验证码</button></div>
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color3" lay-submit lay-filter="submit_authchange">立即更换</button></div>
											</div>
										</div>
									</from>
								</div>
							<div class="layui-tab-item">
									<from class="layui-form">
										<div class="layui-form-item">
											...
											<option value="0">请选择认证的程序</option>
											<?php
											$rs=$DB->query("SELECT * FROM yixi_program WHERE 1 order by id desc");
											while($res = $DB->fetch($rs))
											{
										...
										<div class="layui-form-item">
											<input type="text" name="payqq" placeholder="认证绑定的QQ" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<input type="text" name="payurl" placeholder="旧域名" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<input type="text" name="payurls" placeholder="新域名" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<input type="text" name="paycode" placeholder="验证码" class="layui-input" style="width: 100%;"/>
										</div>
										<div class="layui-form-item">
											<div class="layui-row layui-col-space12">
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color" lay-submit lay-filter="submit_paygetcode">获取验证码</button></div>
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color3" lay-submit lay-filter="submit_paychange">立即更换</button></div>
											</div>
										</div>
									</from>
								</div>
					...
					<?php } else {?>
					<center><font color="red">本站未开启在线更换功能</font></center>
					<?php }?>
					<div class="layui-form">
						<div class="layui-form-item">
							...
							<div class="layui-row layui-col-space12">
								<div class="layui-col-xs6"><a href="?mod=change" class="layui-btn layui-btn-fluid auth_color">在线兑换</a></div>
...
layui.use(['form', 'element'], function () {
	...
	form.on('submit(submit_authgetcode)', function (data) {
		getcode(1);
		return false;
	});
	form.on('submit(submit_paygetcode)', function (data) {
		getcode(2);
		return false;
	});
...
function aa() {
	layer.open({
		...
		content: '<div style="background-color:#393D49;color:#eeeeee;padding:0.5em"><h3 style="text-align:center;padding-top:0.5em">平台公告</h3><hr><?php echo $conf['change_notice'];?><hr/><center>客服QQ：<?php echo $conf['kfqq'];?></center></div>'
	});
};
aa();
<?php }?>
...
function getcode(type){
	...
	$.ajax({
		type : "POST",
		url : "ajax.php?act=getcode&type="+type,
		data : {proid:proid,qq:qq},
		dataType: 'json',
...
function authchange() {
	...
	var urls = $("input[name='authurls']").val();
	var code = $("input[name='authcode']").val();
	var index = layer.msg('正在操作中...', {icon: 16, time: 10 * 1000});
	$.ajax({
		type: "post",
		url: "ajax.php?act=sqgh",
		data : {proid:proid,qq:qq,url:url,urls:urls,code:code},
		dataType: 'json',
		success: function (data) {
			if (data.code == 0) {
				layer.open({
...
function paychange() {
	...
	var urls = $("input[name='payurls']").val();
	var code = $("input[name='paycode']").val();
	var index = layer.msg('正在操作中...', {icon: 16, time: 10 * 1000});
	$.ajax({
		type: "post",
		url: "ajax.php?act=rzgh",
		data : {proid:proid,qq:qq,url:url,urls:urls,code:code},
		dataType: 'json',
		success: function (data) {
			if (data.code == 0) {

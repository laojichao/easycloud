...
if($conf['invite_rebate_open']==1){
	if(isset($_GET['invitecode'])){
		...
		$invite=$DB->get_row("SELECT * FROM yixi_user WHERE invitecode='".$invitecode."'");
		if(!$invite){
			$invitecode='';
		}
	}
	...
	<div class="layui-row layui-col-space8">
		<div class="layui-col-xs12 layui-col-sm8 layui-col-sm-offset2 layui-col-md6 layui-col-md-offset3 misett">
			<div class="layui-card">
				<div class="layui-card-header" style="font-size: 1.5rem;text-align: center;height: 3rem;line-height: 3rem;">在线购买</div>
				<div class="layui-card-body">
					<?php if ($conf['buy_open'] == 1) {?>
					<div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
						<ul class="layui-tab-title" style="text-align: center;margin-bottom: 1rem;">
							<li class="layui-this">购买授权</li>
							<li>购买认证</li>
							<li>购买权限</li>
						</ul>
						<div class="layui-tab-content">
							<div class="layui-tab-item layui-show">
									<form class="layui-form">
										<div class="layui-form-item">
											<select name="authproid" class="layui-input" style="width: 100%;">
										...
										<div class="layui-form-item">
											<input type="text" name="authinvitecode" placeholder="请填写邀请人的邀请码" class="layui-input" style="width: 100%;"/>
										</div>
										<?php }?>
										<div class="layui-form-item">
											<div class="layui-row layui-col-space12">
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color" lay-submit lay-filter="submit_buyauth">立即购买</button></div>
												<div class="layui-col-xs6"><span class="layui-btn layui-btn-fluid auth_color3" onclick="authdesc();">购买介绍</span></div>
											</div>
										</div>
									</from>
								</div>
							<div class="layui-tab-item">
									<from class="layui-form">
										<div class="layui-form-item">
											<select name="payproid" class="layui-input" style="width: 100%;">
											<option value="0">请选择程序</option>
											<?php
											$rs=$DB->query("SELECT * FROM yixi_program WHERE 1 order by id desc");
											while($res = $DB->fetch($rs))
										...
										<?php if ($conf['invite_rebate_open'] == 1) {?>
										<div class="layui-form-item">
											<input type="text" name="payinvitecode" value="<?php echo $invitecode;?>" placeholder="请填写邀请人的邀请码" class="layui-input" style="width: 100%;"/>
										</div>
										<?php }?>
										<div class="layui-form-item">
											<div class="layui-row layui-col-space12">
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color" lay-submit lay-filter="submit_buypay">立即购买</button></div>
												<div class="layui-col-xs6"><span class="layui-btn layui-btn-fluid auth_color3" onclick="paydesc();">购买介绍</span></div>
											</div>
										</div>
									</from>
								</div>
							<div class="layui-tab-item">
									<from class="layui-form">
										<div class="layui-form-item">
											<select name="proid" class="layui-input" style="width: 100%;">
											<option value="0">请选择程序</option>
											<?php
											$rs=$DB->query("SELECT * FROM yixi_program WHERE 1 order by id desc");
											while($res = $DB->fetch($rs))
										...
										<div class="layui-form-item">
											<select name="power" class="layui-input" style="width: 100%;">
											<option value="1">授权商</option>
											<option value="2">超级管理员</option>
											<option value="3">全能管理员</option>
										</div>
										<div class="layui-form-item">
										...
										<?php if ($conf['invite_rebate_open'] == 1) {?>
										<div class="layui-form-item">
											<input type="text" name="invitecode" value="<?php echo $invitecode;?>" placeholder="请填写邀请人的邀请码" class="layui-input" style="width: 100%;"/>
										</div>
										<?php }?>
										<div class="layui-form-item">
											<div class="layui-row layui-col-space12">
												<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color" lay-submit lay-filter="submit_buyuser">立即购买</button></div>
												<div class="layui-col-xs6"><span class="layui-btn layui-btn-fluid auth_color3" onclick="userdesc();">购买介绍</span></div>
											</div>
										</div>
									</from>
								</div>
					...
					<?php } else {?>
					<center><font color="red">本站未开启在线购买功能</font></center>
					<?php }?>
					<div class="layui-form">
						<div class="layui-form-item">
							<div class="layui-row layui-col-space12">
								<div class="layui-col-xs6"><a href="?mod=kmchange" class="layui-btn layui-btn-fluid auth_color">卡密兑换</a></div>
...
layui.use(['form', 'element'], function () {
	...
	form.on('submit(submit_buyauth)', function (data) {
		buyauth();
		return false;
	});
	form.on('submit(submit_buypay)', function (data) {
		buypay();
		return false;
	});
	form.on('submit(submit_buyuser)', function (data) {
		buyuser();
		return false;
	});
});
<?php if ($conf['buy_notice']) {?>
function aa() {
	layer.open({
		type: 1,
		title: false,
		...
		content: '<div style="background-color:#393D49;color:#eeeeee;padding:0.5em"><h3 style="text-align:center;padding-top:0.5em">平台公告</h3><hr><?php echo $conf['buy_notice'];?><hr/><center>客服QQ：<?php echo $conf['kfqq'];?></center></div>'
	});
};
aa();
<?php }?>
...
function authdesc() {
	layer.open({
		title: "购买授权介绍",
		content: '1.购买授权，请先选择程序</br>2.输入邀请码，后台将会将你列入这个邀请人的下级，同时邀请人将会收到你所付的金额的一部分作为奖励发放给您的邀请人，奖励可用于提现和购买商品',
...
function paydesc() {
	layer.open({
		title: "购买认证介绍",
		content: '1.购买易支付域名认证，请先选择程序</br>2.输入邀请码，后台将会将你列入这个邀请人的下级，同时邀请人将会收到你所付的金额的一部分作为奖励发放给您的邀请人，奖励可用于提现和购买商品',
		skin: 'layui-layer-admin',
		btnAlign: 'c',
		btn: '好的',
		icon: 1,
...
function userdesc() {
	layer.open({
		title: "购买权限介绍",
		content: '1.购买权限等级，请先选择程序(购买全能管理员不需要选择程序)</br>2.授权商：可以授权你所选择的授权</br>超级管理员：可以授权你所选择的授权和授权商</br>全能管理员：0成本添加任何授权，授权商，超管</br>3.输入邀请码，后台将会将你列入这个邀请人的下级，同时邀请人将会收到你所付的金额的一部分作为奖励发放给您的邀请人，奖励可用于提现和购买商品',
		skin: 'layui-layer-admin',
		btnAlign: 'c',
		btn: '好的',
		icon: 1,
...
function buyauth() {
	var proid = $("select[name='authproid']").val();
	var name = $("input[name='authname']").val();
	var qq = $("input[name='authqq']").val();
	var url = $("input[name='authurl']").val();
	var invitecode = $("input[name='authinvitecode']").val();
	var index = layer.msg('正在操作中...', {icon: 16, time: 10 * 1000});
	$.ajax({
		type: "post",
		url: "ajax.php?act=buyauth",
		data : {proid:proid,name:name,qq:qq,url:url,invitecode:invitecode},
		dataType: 'json',
		success: function (data) {
			if (data.code == 0) {
				var paymsg = '';
				if(data.pay_alipay>0){
					paymsg+='<div class="layui-col-xs4"><button class="layui-btn layui-btn-fluid auth_color3" onclick="dopay(\'alipay\',\''+data.trade_no+'\')"><img width="15" src="assets/img/alipay.png" class="logo">支付宝</button></div>';
				}
				if(data.pay_qqpay>0){
					paymsg+='<div class="layui-col-xs4"><button class="layui-btn layui-btn-fluid auth_color2" onclick="dopay(\'qqpay\',\''+data.trade_no+'\')"><img width="15" src="assets/img/qqpay.png" class="logo">QQ钱包</button></div>';
				}
				if(data.pay_wxpay>0){
					paymsg+='<div class="layui-col-xs4"><button class="layui-btn layui-btn-fluid auth_color" onclick="dopay(\'wxpay\',\''+data.trade_no+'\')"><img width="15" src="assets/img/wxpay.png" class="logo">微信支付</button></div>';
				}
				layer.open({
					title: "提交订单成功通知",
					content: '<h2>'+data.msg+'</br>金额：'+data.money+'</h2><hr><div class="layui-form"><div class="layui-form-item"><div class="layui-row layui-col-space12">'+paymsg+'</div></div></div>',
					skin: 'layui-layer-admin',
					btnAlign: 'c',
					btn: '取消订单',
					icon: 1,
...
function buypay() {
	var proid = $("select[name='payproid']").val();
	var name = $("input[name='payname']").val();
	var qq = $("input[name='payqq']").val();
	var url = $("input[name='payurl']").val();
	var invitecode = $("input[name='payinvitecode']").val();
	var index = layer.msg('正在操作中...', {icon: 16, time: 10 * 1000});
	$.ajax({
		type: "post",
		url: "ajax.php?act=buypay",
		data : {proid:proid,name:name,qq:qq,url:url,invitecode:invitecode},
		dataType: 'json',
		success: function (data) {
			if (data.code == 0) {
...
function buyuser() {
	...
	$.ajax({
		type: "post",
		url: "ajax.php?act=buyuser",
		data : {proid:proid,user:user,pwd:pwd,qq:qq,power:power,email:email,invitecode:invitecode},
		dataType: 'json',
		success: function (data) {
			if (data.code == 0) {
				var paymsg = '';

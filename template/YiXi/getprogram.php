	...
	<div class="layui-row layui-col-space8">
		<div class="layui-col-xs12 layui-col-sm8 layui-col-sm-offset2 layui-col-md6 layui-col-md-offset3 misett">
			<div class="layui-card">
				<div class="layui-card-header" style="font-size: 1.5rem;text-align: center;height: 3rem;line-height: 3rem;">下载程序源码</div>
				<div class="layui-card-body">
					<?php if ($conf['getprogram_open'] == 1) {?>
					<form class="layui-form">
						...
						<div class="layui-form-item">
							<input type="text" name="qq" placeholder="请填写授权时所填的QQ" lay-verType="tips" class="layui-input" lay-verify="required" style="width: 100%;"/>
						</div>
						<div class="layui-form-item">
							<input type="text" name="code" placeholder="请填写验证码" class="layui-input" style="width: 100%;"/>
						</div>
						<div class="layui-form-item">
							<div class="layui-row layui-col-space12">
								<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color" lay-submit lay-filter="submit_getcode">获取验证码</button></div>
								<div class="layui-col-xs6"><button type="submit" class="layui-btn layui-btn-fluid auth_color3" lay-submit lay-filter="submit_download">立即下载</button></div>
							</div>
							<div class="layui-row layui-col-space12">
								<div class="layui-col-xs6"><a href="?mod=kmchange" class="layui-btn layui-btn-fluid auth_color">在线兑换</a></div>
								<div class="layui-col-xs6"><a href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo $conf['kfqq'];?>&site=qq&menu=yes" class="layui-btn layui-btn-fluid auth_color3">联系QQ</a></div>
							</div>
							<div class="layui-row layui-col-space12">
								<div class="layui-col-xs6"><a href="?mod=change" class="layui-btn layui-btn-fluid auth_color">在线更换</a></div>
								<div class="layui-col-xs6"><a href="?mod=buy" class="layui-btn layui-btn-fluid auth_color3">在线购买</a></div>
							</div>
						</div>
					</from>
					<?php } else {?>
					<center><font color="red">本站未开启在线下载程序源码功能</font></center>
					<?php }?>
...
<script src="../assets/js/qrlogin2.js?ver=<?php echo VERSION ?>"></script>
<script src="../assets/layui/layui.all.js"></script>
<script type="text/javascript">
layui.use(['form', 'element'], function () {
	var form = layui.form;
	var element = layui.element;
	form.on('submit(submit_getcode)', function (data) {
		getcode('download');
		return false;
	});
	form.on('submit(submit_download)', function (data) {
		download();
		return false;
	});
});
<?php if ($conf['getprogram_notice']) {?>
function aa() {
	layer.open({
		type: 1,
		title: false,
		...
		content: '<div style="background-color:#393D49;color:#eeeeee;padding:0.5em"><h3 style="text-align:center;padding-top:0.5em">平台公告</h3><hr><?php echo $conf['getprogram_notice'];?><hr/><center>客服QQ：<?php echo $conf['kfqq'];?></center></div>'
	});
};
aa();
<?php }?>
...
function download() {
	...
	var index = layer.msg('正在验证中...', {icon: 16, time: 10 * 1000});
	$.ajax({
		type: "post",
		url: "ajax.php?act=downfile",
		data : {proid:proid,qq:qq,code:code},
		dataType: 'json',
		success: function (data) {
			if (data.code == 0) {
				layer.open({
					title: "验证成功通知",
					content: '<h2>程序：'+data.name+'<hr>下载QQ：'+data.qq+'</h2><hr>注：下载链接2分钟内有效哦<hr><div class="layui-form"><div class="layui-form-item"><div class="layui-row layui-col-space12"><div class="layui-col-xs6"><a href="'+data.installer+'" class="layui-btn layui-btn-fluid auth_color3">下载完整包</a></div><div class="layui-col-xs6"><a href="'+data.updater+'" class="layui-btn layui-btn-fluid auth_color">下载更新包</a></div></div></div></div>',
					skin: 'layui-layer-admin',
					btnAlign: 'c',
					btn: '好的',
					icon: 1,
